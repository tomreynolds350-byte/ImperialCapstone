...
def _save_round_04_outputs(round_outputs: List[float], out_dir: Path) -> None:
    values = ", ".join(f"np.float64({v!r})" for v in round_outputs)
    text = f"[{values}]\n"
    (out_dir / "round_04_outputs_canonical.txt").write_text(text, encoding="utf-8")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest latest round outputs and propose Round 05 candidates (GP + NN hybrid)."
    )
    parser.add_argument(
        "--inputs-path",
        type=Path,
    ...
    parser.add_argument(
        "--prefix",
        default="round_05",
        help="Output filename prefix (e.g., round_05_explore).",
    )
    parser.add_argument(
        "--skip-ingest",
        action="store_true",
    ...
    if args.skip_ingest:
        ...
        _save_round_04_outputs(parsed.round_outputs, args.out_dir)
    ...
    _write_round_05_outputs(args.out_dir, raw_vectors, portal_strings, debug_info, prefix=args.prefix)
