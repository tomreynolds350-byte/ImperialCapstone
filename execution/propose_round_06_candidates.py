...
def _save_round_04_outputs(round_outputs: List[float], out_dir: Path) -> None:
    ...
    text = f"[{values}]\n"
    (out_dir / "round_05_outputs_canonical.txt").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument(
        "--prefix",
        default="round_06",
        help="Output filename prefix (e.g., round_06_explore).",
    )
    parser.add_argument(
        "--skip-ingest",
        action="store_true",
    ...
    _write_round_06_outputs(args.out_dir, raw_vectors, portal_strings, debug_info, prefix=args.prefix)
